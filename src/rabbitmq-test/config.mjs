 ...
 *
 * 示例（写入项目根目录 .env）：
 *   RABBITMQ_URL=amqp://admin:Admin%40123456@localhost:5672/
 */
...
export async function connect() {
  if (!RABBITMQ_URL) {
    throw new Error(
      '缺少 RABBITMQ_URL。请在 .env 中配置，例如：amqp://admin:Admin%40123456@localhost:5672/\n' +
        '未配置时 amqplib 会默认用 guest/guest，常导致 403 ACCESS_REFUSED。'
    );
  }

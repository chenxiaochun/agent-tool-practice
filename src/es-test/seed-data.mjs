...
async function seedMilvus(collectionName, rows, emb) {
  try {
    ...
    // 给向量字段建 ANN 索引：不建则无法高效做相似度检索
    // - HNSW：图索引，召回/速度平衡好，适合开发与中小规模
    // - L2：欧氏距离（越小越相似）；也可用 COSINE / IP，需与检索侧一致
    // - M：每个节点连边数，越大图越密、召回更好，但建索引更慢、更占内存
    // - efConstruction：建索引时扩展邻居数，越大质量越高、建得越慢
    console.log('创建向量索引...');

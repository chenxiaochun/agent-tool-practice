...
/** 解析路由 JSON；当前网关对 withStructuredOutput(json_object) 要求消息里含 json，故改用普通 invoke */
function parseRoute(raw) {
    ...
    // 解析失败时偏向 complex，避免该检索时误走直答
    return {
        strategy: "complex",
        reason: text || "无法解析路由结果，默认走检索",
    };
}
...
const routeQuestionNode = async (state) => {
    ...
    const response = await model.invoke(`
        ...
        用户问题：${state.question}
    `);
    const route = parseRoute(response.content);
